# ib_pf_ms_bhr.inv_plugins run Role
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: ib_pf_ms_bhr.inv_plugins.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from ib_pf_ms_bhr.inv_plugins
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: ib_pf_ms_bhr.inv_plugins.run
      vars:
        run_x: 42
```
